...
// Initialize the content for the index file and icon types mapping file.
...
`;
  ...
  // Add the icon name and its corresponding type to the mapping file.
  iconMappingFileContent += `  "${iconName}" | `;
});

// Close the icon types mapping file.
iconMappingFileContent += "\n};\n";

// Write the updated index.tsx file.
fs.writeFileSync(path.join(outputDir, "index.tsx"), indexFileContent);

// Write the icon types mapping to the output directory.
fs.writeFileSync(path.join(rootDir, "IconName.ts"), iconMappingFileContent);
